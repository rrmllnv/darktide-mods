...
local ELEMENT_CLASS_NAME = "HudElementRobocopHUD"
local ELEMENT_FILENAME = "RobocopHUD/scripts/mods/RobocopHUD/hud/hud_element_robocop_hud"

local function ensure_element_is_listed()
	mod._robocophud_hud_elements = mod._robocophud_hud_elements or {}
	local pool = mod._robocophud_hud_elements

	for i = 1, #pool do
		local e = pool[i]

		if e and e.class_name == ELEMENT_CLASS_NAME then
			e.filename = ELEMENT_FILENAME
			e.visibility_groups = e.visibility_groups or {
				"alive",
			}
...
local function remove_existing_robocophud_element()
...
UIHudSettings.element_draw_layers[ELEMENT_CLASS_NAME] = 305

remove_existing_robocophud_element()
ensure_element_is_listed()
