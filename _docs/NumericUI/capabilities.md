...
**⚠️ ВАЖНО: Отличие от мода WarpUnboundTimer**
- **NumericUI Peril** - показывает **общую механику опасности** (Warp Charge), которая работает для **всех Псайкеров** постоянно
- **WarpUnboundTimer** - показывает таймер для **конкретного таланта** "Warp Unbound" (psyker_overcharge_stance_infinite_casting)
  - Это талант, который дает временный эффект во время способности "Overcharge Stance"
  - Эффект предотвращает взрыв от Peril на время действия способности
  - Таймер показывает, сколько времени осталось до окончания этого защитного эффекта
- **Это разные вещи:** Peril - это постоянная механика опасности, а Warp Unbound - временный бафф от таланта
