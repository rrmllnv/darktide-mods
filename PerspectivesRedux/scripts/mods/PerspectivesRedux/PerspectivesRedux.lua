...
-- Сохраненные оригинальные значения для восстановления при выгрузке
...
-- Счётчики для оптимизации проверки reasons
local enable_reasons_count = 0
local disable_reasons_count = 0

-- Lookup-таблица для autoswitch событий (предварительно распарсенная)
...
-- Текущая точка обзора (инициализируется после загрузки настроек)
...
-- Проверка есть ли причины для отключения 3P (оптимизированная)
local _has_disable_reason = function()
	return disable_reasons_count > 0
end

-- Проверка есть ли причины для включения 3P (оптимизированная)
...
-- Применить текущую перспективу к юниту
...
-- Отключить 3P по определенной причине
...
-- Включить 3P по определенной причине
...
mod.on_setting_changed = function(id)
	if not is_initialized and id ~= "perspective_transition_time" and id ~= "allow_switching" then
		-- Откладываем применение настроек до инициализации (кроме критичных)
		return
	end
	...
	elseif id == "perspective_transition_time" then
		-- Безопасное изменение глобальных настроек камеры
	...
	elseif string.find(id, OPT_PREFIX_AUTOSWITCH) then
		-- Обновляем lookup-таблицу autoswitch событий
...
local _initialize_settings = function()
	...
	-- Инициализация current_viewpoint на основе настроек
	...
	-- Загрузка всех autoswitch настроек
...
mod.on_unload = function(quitting)
	if not quitting then
		...
		-- Восстанавливаем оригинальные значения переходов камеры
...
-- Общий хук для обработки action_two_hold (объединенная версия)
